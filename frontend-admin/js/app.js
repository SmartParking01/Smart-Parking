...
async function renderStats() {
  ...
  viewEl.innerHTML = `
    <div class="grid-3">
      <div class="stat">
        <div class="stat-icon green"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21h18M7 17v-6M12 17V7M17 17v-4"/></svg></div>
        <div><div class="stat-label">Ocupación actual</div><div class="stat-value">${occupancyRatePercent}%</div></div>
      </div>
      <div class="stat">
        <div class="stat-icon blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="6" width="18" height="15" rx="2"/><path d="M9 6V4a2 2 0 012-2h2a2 2 0 012 2v2"/></svg></div>
        <div><div class="stat-label">Espacios ocupados</div><div class="stat-value">${current.OCCUPIED}</div></div>
      ...
      <div class="stat">
        <div class="stat-icon yellow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="6" width="18" height="15" rx="2"/></svg></div>
        <div><div class="stat-label">Total de espacios</div><div class="stat-value">${current.TOTAL}</div></div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      ${!prediction.available ? `<p class="muted">${prediction.message}</p>` :
        `<table>
          <thead><tr><th>Hora</th><th>Entradas históricas</th><th>Nivel estimado</th></tr></thead>
          <tbody>
    ...
    <div class="card">
      ...
      ${weekdayHistory.length === 0 ? '<p class="muted">Sin datos históricos todavía.</p>' :
        `<table>
          <thead><tr><th>Día</th><th>Entradas registradas</th></tr></thead>
          <tbody>
